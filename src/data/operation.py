...
def import_non_spatial_data_frame(conn, df_to_return_name: str):
    return pd.read_sql(f"SELECT * FROM {df_to_return_name}", conn)
...
# def import_all_non_spatial_data_frame(connection_link: str):
#     conn = sqlite3.connect(connection_link)
#     cursor = conn.cursor()
